...
def run_demo():
    ...
    # Demo Case Selection - using actual case IDs from Kepler.csv
    print_header("STRATEGIC CASE SELECTION")
    print("We selected cases showcasing different mutation types:\n")

    # These are the actual case IDs available in Kepler.csv
    demo_cases = [
        ...
        (8, "Numerical precision", "Badmotorfinger sales: double platinum certification"),
        (10, "Numerical inflation", "Adele Carpool Karaoke: view count discrepancy"),
        (12, "Context preservation", "NFL scheduling: exceptions and flexibility"),
    ]
    ...
    if choice.strip():
        ...
        if 0 <= choice_idx < len(demo_cases):
            selected_case_id = demo_cases[choice_idx][0]
        else:
            print(f"{Colors.RED}Invalid choice, using Case 0{Colors.END}")
            selected_case_id = demo_cases[0][0]
    else:
        selected_case_id = demo_cases[0][0]  # Default to first case
